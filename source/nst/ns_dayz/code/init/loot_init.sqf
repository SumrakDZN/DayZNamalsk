...
_cfgCount = count (configFile >> dayzNam_buildingLoot);
for "_i" from 0 to ((_cfgCount) - 1) do {
	_config = (configFile >> dayzNam_buildingLoot) select _i;
...
dayz_LootBuildings = [];
for "_i" from 0 to (count (configFile >> dayzNam_buildingLoot) - 1) do {
	_type = (configFile >> dayzNam_buildingLoot) select _i;
