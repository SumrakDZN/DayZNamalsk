private["_cfgCount","_config","_configCustomMod","_i","_itemChances","_itemCount","_weighted","_j","_weight","_l","_k","_type","_canZombie","_canLoot"];
dayz_CBLChances = [];
dayz_CBLBase = [];
	...
	if (dayzNam_buildingLoot != "CfgBuildingLootNamalsk") then {
		for "_g" from 0 to ((count _configCustomMod) - 1) do {
			...
			if (_classnameMod == _classname) then {
				...
				_itemChances = [] + getArray (_configCustomMod >> _classname >> "ItemChance");
			};
		};
	};
	if (!_found) then {
